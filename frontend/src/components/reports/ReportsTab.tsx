...
import {
  AttentionTab, buildReportData, COST_KIND_LABELS, CostKind, defaultDateRange, detectPreset, formatDateTime, formatPeriod, LOG_TYPES, LOG_TYPE_LABELS, MACHINE_STATUS_LABELS,
...
const STORAGE_KEY = 'silknode-machine-pro.reports.v2';
const STATUS_VALUES: StatusFilter[] = ['all', 'completed', 'planned'];
const COST_KINDS = Object.keys(COST_KIND_LABELS) as CostKind[];
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

const defaultFilters = (): ReportFilters => ({
  branchId: 'all', machineId: 'all', manufacturer: 'all', machineStatus: 'all', type: 'all', status: 'all', costKind: 'total', ...defaultDateRange(),
});
  ...
  try {
    ...
    return {
      ...
      filters: {
        ...
        costKind: COST_KINDS.includes(f.costKind) ? f.costKind : 'total',
  ...
  return (
    <div className="report-print-area space-y-4 sm:space-y-6 overflow-auto pb-20 px-1 custom-scrollbar">
      ...
      <div className="hidden print:block border-b-2 border-slate-800 pb-3">
        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Silknode Machine Pro · Аналитический отчёт</p>
        ...
        <p className="text-xs text-slate-700 mt-1">
          ...
          {filters.costKind && filters.costKind !== 'total' ? <> · Затраты: <strong>{COST_KIND_LABELS[filters.costKind].toLowerCase()}</strong></> : null}
        </p>
      ...
      <div className="print:hidden bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 sm:space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-3 sm:gap-4">
          ...
          <div>
            <label className={LABEL_CLASS}>Вид затрат</label>
            <select className={SELECT_CLASS} value={filters.costKind ?? 'total'} onChange={e => update({ costKind: e.target.value as CostKind })}>
