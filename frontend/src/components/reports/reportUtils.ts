...
export interface ReportFilters {
  ...
  type: 'all' | LogType;
  status: StatusFilter;
  /** Which part of a work's cost every cost figure shows: labor + parts, labor only or parts only. */
  costKind?: CostKind;
  start: string;
  end: string;
}

export type CostKind = 'total' | 'labor' | 'parts';

export const COST_KIND_LABELS: Record<CostKind, string> = {
  total: 'Работы + запчасти',
  labor: 'Только работы',
  parts: 'Только запчасти',
};

/** Labor / parts / total of one log. Logs saved before the split (no laborCost) count their cost as labor. */
export function logCostParts(log: MaintenanceLog): { labor: number; parts: number; total: number } {
  const labor = log.laborCost ?? log.cost ?? 0;
  const parts = log.partsCost ?? 0;
  return { labor, parts, total: roundTo2(labor + parts) };
}

/**
 * Makes `cost` of every log the part the filter asks for, so every block that sums `cost`
 * follows the same "вид затрат" filter; laborCost/partsCost keep the full breakdown.
 */
export function applyCostKind(logs: MaintenanceLog[], kind: CostKind = 'total'): MaintenanceLog[] {
  return logs.map(log => {
    const { labor, parts, total } = logCostParts(log);
    return { ...log, laborCost: labor, partsCost: parts, cost: kind === 'labor' ? labor : kind === 'parts' ? parts : total };
  });
}

/** Completed works only, like every other cost figure. */
export function costBreakdown(logs: MaintenanceLog[]): { labor: number; parts: number; total: number } {
  return logs.reduce((acc, l) => {
    if (!isCompleted(l)) return acc;
    const c = logCostParts(l);
    return { labor: roundTo2(acc.labor + c.labor), parts: roundTo2(acc.parts + c.parts), total: roundTo2(acc.total + c.total) };
...
export function buildReportData(input: ReportInput, filters: ReportFilters, now: Date = new Date()): ReportData {
  const allLogs = applyCostKind(input.logs, filters.costKind);
  ...
  return {
    ...
    machines,
    logs: filterLogs(allLogs, machineMap, filters),
    prevLogs: prev ? filterLogs(allLogs, machineMap, filters, prev) : null,
    allLogs,
...
/** Price a used part was saved with; logs saved before prices were stored fall back to today's price. */
export function usedPartPrice(used: { partId?: string; name?: string; unitPrice?: number }, lookup: PartLookup): number {
  return used.unitPrice ?? resolvePart(used, lookup)?.unitPrice ?? 0;
}
...
export function partsUsage(logs: MaintenanceLog[], lookup: PartLookup): PartUsageRow[] {
  ...
  logs.forEach(l => {
    ...
    l.partsUsed?.forEach(used => {
      ...
      row.qty += qty;
      row.totalCost = roundTo2(row.totalCost + qty * usedPartPrice(used, lookup));
    });
  });
  // Saved prices can differ between works, so the shown price is the average actually paid.
  rows.forEach(r => (r.unitPrice = r.qty ? roundTo2(r.totalCost / r.qty) : 0));
  return [...rows.values()]
    .filter(r => r.qty > 0)
...
export function monthlyPartsCost(logs: MaintenanceLog[], lookup: PartLookup, range: DateRange): { month: string; label: string; cost: number; qty: number }[] {
  ...
  completed.forEach(l => {
    ...
    l.partsUsed?.forEach(used => {
      const qty = used.quantity || 0;
      point.qty += qty;
      point.cost = roundTo2(point.cost + qty * usedPartPrice(used, lookup));
    });
  });
  return [...points.values()];
}
...
export interface PowerSummary {
  ...
  /** Machines without amperage; amperage is optional and they count as 0 A. */
  missingCount: number;
  withAmperage: number;
  /** Over all non-retired machines, the ones without amperage counting as 0 A. */
  averageAmps: number;
  maxAmps: number;
  maxMachine: { id: string; name: string } | null;
}

/** Retired machines are excluded entirely; machines without amperage count as 0 A. */
export function powerSummary(machines: Machine[]): PowerSummary {
  ...
  return {
    ...
    missingCount,
    withAmperage,
    averageAmps: machineCount ? roundTo2(totalAmps / machineCount) : 0,
...
export function usageByMachine(logs: MaintenanceLog[], lookup: PartLookup, machineMap: Map<string, Machine>): { machineId: string; name: string; qty: number; cost: number; works: number }[] {
  ...
  logs.forEach(l => {
    ...
    l.partsUsed.forEach(u => {
      ...
      row.qty += qty;
      row.cost = roundTo2(row.cost + qty * usedPartPrice(u, lookup));
    });
...
export interface AttentionGroup {
  id: 'repair' | 'overdue' | 'lowStock' | 'noPrice';
  title: string;
  tab: AttentionTab;
  tone: 'danger' | 'warning';
