...
import {
  ...
  formatPercent, isCompleted, isRetired, lastMaintenanceByMachine, logCostParts, LOG_TYPE_BADGE, LOG_TYPE_LABELS, MACHINE_STATUS_LABELS, MACHINE_STATUSES,
  ...
  return (
    <section className="rounded-2xl border-2 border-blue-200 bg-blue-50/30 p-3 sm:p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <KpiCard label="Ток" value={formatAmps(Number(machine.amperage) || 0)}
          hint={Number(machine.amperage) > 0 ? undefined : 'Ампераж не указан — считается 0 А'} />
      ...
      <Panel
        ...
        actions={canExport && (
          <CsvButton filename={`istoriya_rabot_${machine.name}`} disabled={!card.logs.length}
            headers={['Дата', 'Вид работ', 'Статус', 'Исполнитель', 'Описание', 'Работы, $', 'Запчасти, $', 'Итого, $']}
            rows={() => card.logs.map(l => {
              const c = logCostParts(l);
              return [formatDateKey(toLocalDateKey(l.date)), LOG_TYPE_LABELS[l.type], isCompleted(l) ? 'Выполнено' : 'Запланировано', l.technicianName, l.notes, c.labor, c.parts, c.total];
            })} />
        )}
      >
              ...
              <tbody className="divide-y divide-slate-50">
                {card.logs.map(l => (
                  <tr key={l.id}>
                    ...
                    <td className={`px-4 sm:px-6 py-3 text-right whitespace-nowrap ${isCompleted(l) ? '' : 'text-slate-400'}`}>
                      <p className="font-mono font-bold">{formatMoney(logCostParts(l).total)}</p>
                      <p className="text-[10px] text-slate-400">работы {formatMoney(logCostParts(l).labor)} · запч. {formatMoney(logCostParts(l).parts)}</p>
                    </td>
                  </tr>
                ))}
              </tbody>
