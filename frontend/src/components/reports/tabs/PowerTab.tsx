...
import { Building2, ExternalLink, Info, Zap } from 'lucide-react';
import {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
        ...
        <KpiCard label="Без ампеража" value={formatNumber(summary.missingCount)}
          hint={summary.missingCount > 0 ? 'Ампераж необязателен: такие станки считаются как 0 А' : 'Ампераж указан у всех станков'} />
        <KpiCard label="Средний ток на станок" value={formatAmps(summary.averageAmps)}
          hint={`По ${summary.machineCount} ${pluralRu(summary.machineCount, ['станку', 'станкам', 'станкам'])}, без ампеража = 0 А`} />
        ...
        <Panel
          ...
          actions={canExport && (
            <CsvButton
              ...
              headers={['Филиал', 'Станков (без списанных)', 'Всего, А', 'В работе, А', 'Отключено, А', 'Без ампеража (0 А)', 'Доля, %']}
              rows={() => [
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className={THEAD_ROW}>
                    ...
                    <th className="px-4 py-3 text-right">Без ампеража</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {stats.byBranch.rows.map(r => (
                    <tr key={r.branchId} className="hover:bg-blue-50/20 transition-colors">
                      ...
                      <td className={`${TD} text-right font-mono ${r.missingCount ? 'text-slate-600' : 'text-slate-300'}`}>{r.missingCount}</td>
                ...
                <tfoot>
                  <tr className={TFOOT_ROW}>
                    ...
                    <td className={`${TD} text-right font-mono`}>{summary.missingCount}</td>
      ...
      <Panel
        title={`Станки без ампеража (0 А): ${stats.missing.length}`}
        icon={Zap}
                    ...
                    {onOpenMachine && (
                      <td className="px-4 sm:px-6 py-3 text-right print:hidden">
                        <button type="button" onClick={() => onOpenMachine(m.id)} className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-600 hover:underline cursor-pointer">
                          <ExternalLink className="w-3.5 h-3.5" />Указать ампераж
                        </button>
                      </td>
                    )}
                  </tr>
      ...
      <p className="flex items-start gap-2 text-[11px] text-slate-500 leading-relaxed bg-slate-50 border border-slate-200 rounded-xl p-3">
        ...
        Суммарный ток приблизителен и корректен для оборудования одного напряжения. Списанные станки не учитываются; ампераж необязателен — станки без него считаются как 0 А.
      </p>
    </div>
  );
}
