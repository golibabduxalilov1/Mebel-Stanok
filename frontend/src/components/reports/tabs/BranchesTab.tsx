                ...
                <tbody className="divide-y divide-slate-50">
                  {details.machines.map(({ machine, residual, cost }) => (
                    <tr key={machine.id} className="hover:bg-blue-50/20">
                      ...
                      <td className={`${TD} text-right font-mono whitespace-nowrap ${Number(machine.amperage) > 0 ? '' : 'text-slate-400'}`}
                        title={Number(machine.amperage) > 0 ? undefined : 'Ампераж не указан — считается 0 А'}>
                        {formatAmps(Number(machine.amperage) || 0)}
                      </td>
