    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <KpiCard label={`Суммарный ток (${branchLabel})`} value={formatAmps(stats.power.totalAmps)} onClick={() => onNavigate('power')}
            hint={
              <>
                ...
                {stats.power.missingCount > 0 && <span className="text-amber-600 font-bold"> · без ампеража: {stats.power.missingCount}</span>}
              </>
            } />
        )}
      </div>
      ...
      <Panel title="Требует внимания" icon={AlertTriangle} iconClass="text-amber-500">
        {stats.attention.length === 0 ? (
          <EmptyState text="Проблем не найдено: нет станков в ремонте, просрочек, дефицита и пробелов в данных" compact />
        ) : (
